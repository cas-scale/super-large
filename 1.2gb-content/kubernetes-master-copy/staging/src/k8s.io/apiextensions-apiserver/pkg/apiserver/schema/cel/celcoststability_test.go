...
func TestCelCostStability(t *testing.T) {
	cases := []struct {
		...
		{name: "ext lists version 3",
			...
			schema: schemas(listType(&integerType), listType(&integerType)),
			expectCost: map[string]int64{
				`lists.range(4) == [0,1,2,3]`:      16,
				`lists.range(0) == []`:             11,
				`[5,1,2,3].reverse() == [3,2,1,5]`: 16,
				`[].reverse() == []`:               11,
				`[1].reverse() == [1]`:             13,
				`['are', 'you', 'as', 'bored', 'as', 'I', 'am'].reverse() == ['am', 'I', 'as', 'bored', 'as', 'you', 'are']`: 19,
				`[false, true, true].reverse().reverse() == [false, true, true]`:                                             29,
				`[1,2,3,4].slice(0, 4) == [1,2,3,4]`:                                                                         16,
				`[1,2,3,4].slice(0, 0) == []`:                                                                                11,
				`[1,2,3,4].slice(1, 1) == []`:                                                                                11,
				`[1,2,3,4].slice(4, 4) == []`:                                                                                11,
				`[1,2,3,4].slice(1, 3) == [2, 3]`:                                                                            14,
				`dyn([]).flatten() == []`:                                                                                    11,
				`dyn([1,2,3,4]).flatten() == [1,2,3,4]`:                                                                      16,
				`[].sort() == []`:                                                                                            11,
				`[1].sort() == [1]`:                                                                                          14,
				`[4, 3, 2, 1].sort() == [1, 2, 3, 4]`:                                                                        44,
				`["d", "a", "b", "c"].sort() == ["a", "b", "c", "d"]`:                                                        45,
				`[].sortBy(e, e) == []`:                                                                                      14,
				`["a"].sortBy(e, e) == ["a"]`:                                                                                30,
				`[-3, 1, -5, -2, 4].sortBy(e, -(e * e)) == [-5, 4, -3, -2, 1]`:                                               145,
				`[-3, 1, -5, -2, 4].map(e, e * 2).sortBy(e, -(e * e)) == [-10, 8, -6, -4, 2]`:                                216,
				`lists.range(3).sortBy(e, -e) == [2, 1, 0]`:                                                                  89,
				`["a", "c", "b", "first"].sortBy(e, e == "first" ? "" : e) == ["first", "a", "b", "c"]`:                      104,
				`[{'name': 'foo'}, {'name': 'bar'}, {'name': 'baz'}].sortBy(e, e.name) == [{'name': 'bar'}, {'name': 'baz'}, {'name': 'foo'}]`: 75,
				`[].distinct() == []`:                                                                        11,
				`[1].distinct() == [1]`:                                                                      14,
				`[-2, 5, -2, 1, 1, 5, -2, 1].distinct() == [-2, 5, 1]`:                                       140,
				`['c', 'a', 'a', 'b', 'a', 'b', 'c', 'c'].distinct() == ['c', 'a', 'b']`:                     146,
				`[[1], [1], [2]].distinct() == [[1], [2]]`:                                                   30,
				`[{'name': 'a'}, {'name': 'b'}, {'name': 'a'}].distinct() == [{'name': 'a'}, {'name': 'b'}]`: 30,
			},
		},
	}
